...
public class Impex 
{
	...
	public static void exportImage(Image img, String filename) throws IOException
    {
        ...
        if(width != 0 && height != 0 && numbands != 0)
        {
        	if(numbands == 1)
        	{
                if(CLibrary.INSTANCE.vigra_exportgrayimage_c(img.getBand(0), width, height, filename) != 0)
				{
        	...
        	{
                if(CLibrary.INSTANCE.vigra_exportrgbimage_c(img.getBand(0), img.getBand(1), img.getBand(2), width, height, filename) != 0)
				{
